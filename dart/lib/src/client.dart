...
class CpiinflationValidationException implements Exception {
  final List<String> errors;

  CpiinflationValidationException(this.errors);

  @override
  String toString() => 'CpiinflationValidationException: ${errors.join("; ")}';
}
...
/// CPI Inflation API client.
///
/// For more information, visit: https://apiverve.com/marketplace/cpiinflation?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [date] - Optional date in YYYY-MM format for historical lookup. Omit for current data. [format: date]
/// * [category] - Optional category filter: all, food, housing, apparel, transportation, medical, recreation, education, gasoline, electricity
class CpiinflationClient {
  final String apiKey;
  ...
  CpiinflationClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/cpiinflation',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CpiinflationValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CpiinflationValidationException(errors);
    }
  }

  /// Execute a request to the CPI Inflation API.
  ///
  /// Parameters are validated before sending the request.
  Future<CpiinflationResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CpiinflationException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CpiinflationResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CpiinflationException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CpiinflationException('Resource not found');
      } else {
        throw CpiinflationException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CpiinflationException) rethrow;
      throw CpiinflationException('Request failed: $e');
    }
  }
...
/// Exception thrown by the CPI Inflation API client.
class CpiinflationException implements Exception {
  final String message;

  CpiinflationException(this.message);

  @override
  String toString() => 'CpiinflationException: $message';
}
